...
const PlayerIdleState = preload("res://scripts/player/states/player_idle_state.gd")
const PlayerAirState = preload("res://scripts/player/states/player_air_state.gd")
const PlayerDashState = preload("res://scripts/player/states/player_dash_state.gd")
const PlayerWallState = preload("res://scripts/player/states/player_wall_state.gd")
...
@export_group("Necesary Child Nodes")
...
func _ready():
	...
	if state_machine:
		var states: Array[State] = [
			...
			PlayerAirState.new(self),
			PlayerDashState.new(self),
			PlayerWallState.new(self),
		]
...
func try_start_dash() -> bool:
	if rolling or dashCount <= 0 or not dashTap:
		return false
	if eightWayDash:
		var input_direction = Input.get_vector("left", "right", "up", "down")
		var d_time = 0.0625 * dashLength
		_dashingTime(d_time)
		_pauseGravity(d_time)
		velocity = dashMagnitude * input_direction
		if (not rightHold and not leftHold and not downHold and not upHold) and wasMovingR:
			velocity.x = dashMagnitude
		elif (not rightHold and not leftHold and not downHold and not upHold) and not wasMovingR:
			velocity.x = -dashMagnitude
		dashCount -= 1
		movementInputMonitoring = Vector2(false, false)
		_inputPauseReset(d_time)
		return true
	if twoWayDashVertical:
		var d_time_v = 0.0625 * dashLength
		if upHold and downHold:
			_placeHolder()
		elif upHold:
			_dashingTime(d_time_v)
			_pauseGravity(d_time_v)
			velocity.x = 0
			velocity.y = -dashMagnitude
			dashCount -= 1
			movementInputMonitoring = Vector2(false, false)
			_inputPauseReset(d_time_v)
			return true
		elif downHold:
			_dashingTime(d_time_v)
			_pauseGravity(d_time_v)
			...
			_inputPauseReset(d_time_v)
			return true
	if twoWayDashHorizontal:
		var d_time_h = 0.0625 * dashLength
		if wasPressingR and not (upHold or downHold):
			velocity.y = 0
			velocity.x = dashMagnitude
			_pauseGravity(d_time_h)
			_dashingTime(d_time_h)
			dashCount -= 1
			movementInputMonitoring = Vector2(false, false)
			_inputPauseReset(d_time_h)
			return true
...
func reset_ground_counters() -> void:
	jumpCount = jumps
	if coyoteTime > 0:
		coyoteActive = true
	else:
